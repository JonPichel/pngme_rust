...
impl Display for PngError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            PngError::InvalidLength => write!(f, "Invalid length!"),
            PngError::InvalidHeader(bytes) => write!(f, "Invalid header {:?}!", bytes),
            PngError::ChunkNotFound(chunk_type) => write!(f, "Chunk not found {}!", chunk_type),
        }
    }
}
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        if let Err(error) = reader.read_exact(&mut header) {
            return if error.kind() == ErrorKind::UnexpectedEof {
                Err(Box::new(PngError::InvalidLength))
            } else {
                Err(Box::new(error))
            }
        }
        ...
        while let Ok(()) = reader.read_exact(&mut len) {
            ...
            let chunk_bytes: Vec<u8> = len.into_iter()
...
impl Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}",
            self.chunks.iter().map(Chunk::to_string).collect::<Vec<String>>().join(", "))
    }
}
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let index = self.chunks.iter()
            ...
            .ok_or(PngError::ChunkNotFound(chunk_type))?;
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        for chunk in self.chunks().iter() {
            bytes.append(&mut chunk.as_bytes().clone());
        }
        bytes
    }
}
